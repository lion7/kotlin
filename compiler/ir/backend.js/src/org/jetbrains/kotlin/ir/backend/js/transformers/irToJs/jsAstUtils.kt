...
private fun IrDeclarationWithName.originalNameForUseInSourceMap(policy: SourceMapNamesPolicy): String? {
    ...
    when (this) {
        ...
        is IrVariable -> if (origin != IrDeclarationOrigin.DEFINED && origin != IrDeclarationOrigin.FOR_LOOP_VARIABLE) {
            // We don't want to generate name mappings for local variables generated by the compiler.
            return null
        }

        is IrValueParameter -> if (origin.isSynthetic && origin != IrDeclarationOrigin.CONTINUATION) {
            return null
        }
    }
