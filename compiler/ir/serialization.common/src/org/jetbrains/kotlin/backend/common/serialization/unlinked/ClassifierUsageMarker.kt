...
internal class ClassifierUsageMarker(
    ...
    private fun IrClassifierSymbol.exploreSymbol(visitedSymbols: MutableSet<IrClassifierSymbol>): LinkedClassifierSymbolStatus {
        linkedClassifierSymbols[this]?.let { status ->
            // Already explored and registered symbol.
            return status
        }

        val partiallyLinkedStatus: Partially? = when {
            !isBound || hasDescriptor && descriptor is NotFoundClasses.MockClassDescriptor -> MissingOwnerDeclaration
        ...
        return if (partiallyLinkedStatus != null) {
            // TODO: patch
            linkedClassifierSymbols.registerPartiallyLinked(this, partiallyLinkedStatus)
        } else
            linkedClassifierSymbols.registerFullyLinked(this)
    }

    fun IrClassifierSymbol.isPartiallyLinkedClassifier(): Boolean = exploreSymbol(visitedSymbols = hashSetOf()).isPartiallyLinked
