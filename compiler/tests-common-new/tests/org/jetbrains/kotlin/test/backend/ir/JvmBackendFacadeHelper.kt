...
class JvmBackendFacadeHelper(val testServices: TestServices) {
    ...
    fun transform(
        ...
        fun sourceFileInfos(irFile: IrFile, allowNestedMultifileFacades: Boolean): List<SourceFileInfo> =
            when (val fileEntry = irFile.fileEntry) {
                ...
                is NaiveSourceBasedFileEntryImpl -> {
                    val sourceFile = sourceFiles.find { it.path == fileEntry.name }
        ...
        return BinaryArtifacts.Jvm(
            state.factory,
            backendInput.irModuleFragment.files.flatMap { sourceFileInfos(it, allowNestedMultifileFacades = true) }
        )
    }
}