...
class FirResolvedTypesVerifier(testServices: TestServices) : FirAnalysisHandler(testServices, failureDisablesNextSteps = true) {
    ...
    override fun processModule(module: TestModule, info: FirOutputArtifact) {
        val visitor = Visitor(info.session)
    ...
    private inner class Visitor(val session: FirSession) : FirDefaultVisitor<Unit, FirElement>() {
        ...
        private fun checkConeType(type: ConeKotlinType): ConeTypeStatus? {
            val substitutor = CountingSubstitutor()
            substitutor.substituteOrNull(type)
            return when {
                substitutor.stubTypeFound -> ConeTypeStatus.StubFound
                substitutor.typeVariableFound -> ConeTypeStatus.TypeVariableFound
                else -> null
            }
        }

        private inner class CountingSubstitutor : AbstractConeSubstitutor(session.typeContext) {
