...
import org.jetbrains.kotlin.fir.analysis.jvm.checkers.declaration.FirJvmNameAndExposeChecker.isInlineThatRequiresMangling
import org.jetbrains.kotlin.fir.analysis.jvm.checkers.declaration.FirJvmNameAndExposeChecker.isRenamableFunction
...
object FirJvmNameAndExposeChecker : FirBasicDeclarationChecker() {
    ...
    fun checkJvmName(jvmName: FirAnnotation, declaration: FirDeclaration,context: CheckerContext, reporter: DiagnosticReporter) {
        val nameParameter = jvmName.findArgumentByName(StandardClassIds.Annotations.ParameterNames.jvmNameName) ?: return

        if (nameParameter.typeRef.coneType != context.session.builtinTypes.stringType.type) {
            return
        }
    ...
    fun checkJvmExpose(jvmName: FirAnnotation, declaration: FirDeclaration,context: CheckerContext, reporter: DiagnosticReporter) {
        val nameParameter = jvmName.findArgumentByName(StandardClassIds.Annotations.ParameterNames.jvmNameName) ?: return
