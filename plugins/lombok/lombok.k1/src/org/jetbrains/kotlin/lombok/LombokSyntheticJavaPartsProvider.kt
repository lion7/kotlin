...
class LombokSyntheticJavaPartsProvider(config: LombokConfig) : SyntheticJavaPartsProvider {
    ...
    override fun generateStaticFunctions(thisDescriptor: ClassDescriptor, name: Name, result: MutableCollection<SimpleFunctionDescriptor>) {
        ...
        functions.mapNotNull { it.returnType }
            .mapNotNull{ returnType -> syntheticParts.nestedClasses.firstOrNull { it == returnType.constructor.declarationDescriptor }}
    ...
    override fun getNestedClassNames(thisDescriptor: ClassDescriptor): List<Name> {
        return getSyntheticParts(thisDescriptor).nestedClasses.map { it.name }
    }
    context(LazyJavaResolverContext)
    override fun getNestedClass(thisDescriptor: ClassDescriptor, name: Name): JavaClass? {
        return getSyntheticParts(thisDescriptor).nestedClasses.firstOrNull { it.name == name }?.jClass
    }
    ...
    override fun generateNestedClass(
        ...
        result += getSyntheticParts(thisDescriptor).nestedClasses.filter { it.name == name }
    }
